...
function getActionIcon(label: string) {
  ...
  if (lower.includes('send') || lower.includes('webhook')) return Send;
  return Globe;
}
  ...
  return (
    ...
    >
      ...
      <Handle
        ...
        className="!h-3 !w-3 !rounded-full !border-2 !border-violet-500 !bg-background"
      />

      {/* Top row: Icon + category label */}
      <div className="mb-1.5 flex items-center gap-2">
        <Icon className="h-4 w-4 text-violet-500" />
        <span className="text-[10px] font-semibold uppercase tracking-wider text-violet-500/70">
          Action
        </span>
      </div>
